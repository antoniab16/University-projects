...
void UI::run_administartor(){
    while(1){
        ...
        if(option_admin == "0")
            ...
            controller.writeToFile(file_read);
        }
...
void UI::run() {
    ...
    cout<<"Read from memory or from file?"<<endl;
    cout<<">>>";
    cin>>firstOption;
    if(firstOption == "file") {
        cout << "Enter filename from where to read: ";
        cin >> file_read;
        controller.setFile(file_read);
    }
    else{

    }
    cout << "Enter filename where to write HTML or CSV: ";
    cin >> filename;
